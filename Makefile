	...
	for _f in $(srctexfiles); do \
		if [ -z "$$_rev2" ]; then \
			if git show "$$_rev1:./$$_f" >/dev/null 2>&1; then \
				$(MAKE) -f $(Makefile) $${_f%.*}-diff.$(default_target); \
			fi; \
		else \
...
.PHONY : all check clean diff dist dvi eps fmt help mostlyclean pdf ps prerequisite upgrade watch FORCE
...
%-diff.$(default_target): %.tex FORCE
	...
	if [ -z "$$_rev2" ]; then \
		if git show "$$_rev1:./$<" >/dev/null 2>&1; then \
		...
		if git show "$$_rev1:./$<" >/dev/null 2>&1 && git show "$$_rev2:./$<" >/dev/null 2>&1; then \
			$(call expand_latexdiff_repo,$$_rev1); \
